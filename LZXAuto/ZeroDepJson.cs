...
namespace ZeroDep
{
    ...
    public static class Json
    {
        ...
        {
            "yyyy'-'MM'-'dd'T'HH':'mm':'ss'.'fff'Z'", "yyyy'-'MM'-'dd'T'HH':'mm':'ss'Z'", "yyyy'-'MM'-'dd'T'HH':'mm'Z'",
        ...
        private static object CreateInstance(object target, Type type, int elementsCount, JsonOptions options,
            object value)
        {
            try
            {
            ...
            catch (Exception e)
            {
                HandleException(
                    new JsonException(
                        "JSO0001: JSON error detected. Cannot create an instance of the '" + type.Name + "' type.", e),
                    options);
                return null;
            }
        }

        private static ListObject GetListObject(Type type, JsonOptions options, object target, object value,
            IDictionary dictionary, string key)
        {
            ...
            if (type.IsGenericType)
                if (type.GetGenericTypeDefinition() == typeof(ICollection<>))
                    return (ListObject)Activator.CreateInstance(
                        typeof(ICollectionTObject<>).MakeGenericType(type.GetGenericArguments()[0]));
        ...
        private static void ApplyToListTarget(object target, IEnumerable input, ListObject list, JsonOptions options)
        {
            ...
            {
                if (list.Context != null) list.Context["action"] = "clear";
                list.Clear();
            }

            if (list.Context != null) list.Context.Clear();
        }
...
#pragma warning restore IDE0083 // Use pattern matching
        ...
        public static object ChangeType(object target, object value, Type conversionType, JsonOptions options = null)
        {
            ...
            if (!(value is string))
            {
                if (conversionType.IsArray)
                    if (value is IEnumerable en)
                    {
                        ...
                        foreach (var obj in en) list.Add(ChangeType(target, obj, elementType, options));

                        if (elementType != null)
                        {
                            var array = Array.CreateInstance(elementType, list.Count);
                            if (array != null) Array.Copy(list.ToArray(), array, list.Count);
                            return array;
                        }
                    }
                ...
                if (lo != null)
                    if (value is IEnumerable en)
                    {
                        lo.List = CreateInstance(target, conversionType, en is ICollection coll ? coll.Count : 0,
                            options, value);
            ...
            if (value is IDictionary dic)
            {
                ...
                if (instance != null) Apply(dic, instance, options);
                return instance;
            }
            ...
            if (conversionType == typeof(byte[]) && value is string str)
                if (options.SerializationOptions.HasFlag(JsonSerializationOptions.ByteArrayAsBase64))
                    ...
                    {
                        HandleException(
                            new JsonException("JSO0013: JSON deserialization error with a base64 array as string.", e),
                            options);
                        return null;
                    }
        ...
        private static object[] ReadArray(TextReader reader, JsonOptions options)
        {
            ...
            do
            {
                ...
                if (!Convert.IsDBNull(value)) list.Add(value);
                if (arrayEnd)
                    return list.ToArray();
        ...
        {
            if (pos < 0)
                return new JsonException("JSO0002: JSON deserialization error detected. Expecting '" + c +
                                         "' character.");

            return new JsonException("JSO0003: JSON deserialization error detected at position " + pos +
                                     ". Expecting '" + c + "' character.");
        }

        private static JsonException GetUnexpectedCharacterException(long pos, char c)
        {
            if (pos < 0)
                return new JsonException("JSO0004: JSON deserialization error detected. Unexpected '" + c +
                                         "' character.");

            return new JsonException("JSO0005: JSON deserialization error detected at position " + pos +
        ...
        private static JsonException GetExpectedHexaCharacterException(long pos)
        {
            if (pos < 0)
                return new JsonException(
                    "JSO0006: JSON deserialization error detected. Expecting hexadecimal character.");

            return new JsonException("JSO0007: JSON deserialization error detected at position " + pos +
                                     ". Expecting hexadecimal character.");
        }

        private static JsonException GetTypeException(long pos, string typeName, Exception inner)
        {
            if (pos < 0)
                return new JsonException("JSO0010: JSON deserialization error detected for '" + typeName + "' type.",
                    inner);

            return new JsonException(
                "JSO0011: JSON deserialization error detected for '" + typeName + "' type at position " + pos + ".",
                inner);
        }

        private static JsonException GetEofException(char c)
        {
            return new JsonException("JSO0012: JSON deserialization error detected at end of text. Expecting '" + c +
                                     "' character.");
        }
        ...
        private static ISerializable ReadSerializable(TextReader reader, JsonOptions options, string typeName,
            Dictionary<string, object> values)
        {
            ...
            foreach (var kvp in values) info.AddValue(kvp.Key, kvp.Value);
        ...
        {
            return ReadValue(reader, options, false, out var _);
        }
        ...
        private static object ReadNew(TextReader reader, JsonOptions options, out bool arrayEnd)
        {
            ...
            if (text.StartsWith(_dateStartJs) && text.EndsWith(_dateEndJs))
                if (long.TryParse(
                        text.Substring(_dateStartJs.Length, text.Length - _dateStartJs.Length - _dateEndJs.Length),
                        out var l))
        ...
        public static bool TryParseDateTime(string text, DateTimeStyles styles, out DateTime dt)
        {
            ...
            if (text.EndsWith("Z", StringComparison.OrdinalIgnoreCase))
                if (DateTime.TryParseExact(text, _dateFormatsUtc, CultureInfo.InvariantCulture,
                ...
                text[16] == ':')
            {
                ...
                if (tz >= 0)
                {
                    ...
                    {
                        if (offsetHours != 0) dt = dt.AddHours(offsetHours);

                        if (offsetMinutes != 0) dt = dt.AddMinutes(offsetMinutes);
                        return true;
                    }
                }
                else
                {
                    if (DateTime.TryParseExact(text, "s", null,
            ...
            string ticks = null;
            if (text.StartsWith(_dateStartJs) && text.EndsWith(_dateEndJs))
                ticks = text.Substring(_dateStartJs.Length, text.Length - _dateStartJs.Length - _dateEndJs.Length)
                    .Trim();
            else if (text.StartsWith(_dateStart2, StringComparison.OrdinalIgnoreCase) &&
                     text.EndsWith(_dateEnd2, StringComparison.OrdinalIgnoreCase))
        ...
        private static ushort ReadX4(TextReader reader, JsonOptions options)
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                if (reader.Peek() < 0)
                {
                    HandleException(
                        new JsonException(
                            "JSO0008: JSON deserialization error detected at end of stream. Expecting hexadecimal character."),
                        options);
                    return 0;
                }
        ...
        /// <param name="options">The options to use.</param>
        public static void WriteValue(TextWriter writer, object value, IDictionary<object, object> objectGraph,
            JsonOptions options = null)
        {
            ...
            if (value is DateTimeOffset dto)
            {
                ...
                else if (options.SerializationOptions.HasFlag(JsonSerializationOptions.DateTimeOffsetFormatCustom) &&
                         !string.IsNullOrEmpty(options.DateTimeOffsetFormat))
                {
            ...
            if (value is DateTime dt)
            {
                ...
                else if (options.SerializationOptions.HasFlag(JsonSerializationOptions.DateFormatCustom) &&
                         !string.IsNullOrEmpty(options.DateTimeFormat))
                {
            ...
            {
                WriteUnescapedString(writer,
                    options.GuidFormat != null ? guid.ToString(options.GuidFormat) : guid.ToString());
                return;
            }
        ...
        public static long WriteBase64Stream(TextWriter writer, Stream stream, IDictionary<object, object> objectGraph,
            JsonOptions options = null)
        {
            if (writer == null)
                throw new ArgumentNullException(nameof(writer));
        ...
        {
            if (obj is IOptionsHolder holder) holder.Options = options;
        }
            ...
            foreach (var t in type.GetInterfaces())
                if (t.IsGenericType)
                    if (typeof(IEnumerable<>).IsAssignableFrom(t.GetGenericTypeDefinition()))
                    {
                        ...
                        {
                            var kvp = args[0];
                            if (kvp.IsGenericType &&
                                typeof(KeyValuePair<,>).IsAssignableFrom(kvp.GetGenericTypeDefinition()))
                            {
        ...
        /// <param name="objectGraph">The object graph.</param>
        /// <param name="options">The options to use.</param>
        public static void WriteArray(TextWriter writer, Array array, IDictionary<object, object> objectGraph,
            JsonOptions options = null)
        {
            if (writer == null)
                throw new ArgumentNullException(nameof(writer));
...
#pragma warning restore CA1825 // Avoid zero-length array allocations
        ...
        private static void WriteArray(TextWriter writer, Array array, IDictionary<object, object> objectGraph,
            JsonOptions options, int[] indices)
        {
            var newIndices = new int[indices.Length + 1];
            for (var i = 0; i < indices.Length; i++) newIndices[i] = indices[i];
        ...
        /// <param name="objectGraph">The object graph.</param>
        /// <param name="options">The options to use.</param>
        public static void WriteEnumerable(TextWriter writer, IEnumerable enumerable,
            IDictionary<object, object> objectGraph, JsonOptions options = null)
        {
            if (writer == null)
                throw new ArgumentNullException(nameof(writer));
        ...
        /// <param name="objectGraph">The object graph.</param>
        /// <param name="options">The options to use.</param>
        public static void WriteDictionary(TextWriter writer, IDictionary dictionary,
            IDictionary<object, object> objectGraph, JsonOptions options = null)
        {
            if (writer == null)
                throw new ArgumentNullException(nameof(writer));
        ...
        private static void WriteSerializable(TextWriter writer, ISerializable serializable,
            IDictionary<object, object> objectGraph, JsonOptions options)
        {
        ...
        /// <param name="objectGraph">The object graph.</param>
        /// <param name="options">The options to use.</param>
        public static void WriteObject(TextWriter writer, object value, IDictionary<object, object> objectGraph,
            JsonOptions options = null)
        {
            if (writer == null)
                throw new ArgumentNullException(nameof(writer));
            ...
            var useISerializable = options.SerializationOptions.HasFlag(JsonSerializationOptions.UseISerializable) ||
                                   ForceSerializable(value);
            if (useISerializable) serializable = value as ISerializable;
        ...
        /// <param name="objectGraph">The object graph.</param>
        /// <param name="options">The options to use.</param>
        public static void WriteNameValue(TextWriter writer, string name, object value,
            IDictionary<object, object> objectGraph, JsonOptions options = null)
        {
            if (writer == null)
                throw new ArgumentNullException(nameof(writer));
        ...
        private static T GetAttribute<T>(this PropertyDescriptor descriptor) where T : Attribute
        {
        ...
        public class MemberDefinition
        {
            ...
            public virtual object GetOrCreateInstance(object target, int elementsCount, JsonOptions options = null)
            {
                ...
                if (targetValue == null || (targetValue is Array array && array.GetLength(0) < elementsCount))
                {
                    ...
                    if (targetValue != null) Accessor.Set(target, targetValue);
                }
            ...
            /// <param name="options">The options.</param>
            public virtual void ApplyEntry(IDictionary dictionary, object target, string key, object value,
                JsonOptions options = null)
            {
            ...
            public virtual bool IsNullDateTimeValue(object value)
            {
            ...
            public virtual bool EqualsDefaultValue(object value)
            {
                return AreValuesEqual(DefaultValue, value);
            }
        ...
        private sealed class KeyValueTypeEnumerator : IDictionaryEnumerator
        {
            ...
            public DictionaryEntry Entry
            {
                get
                {
                    ...
                    return new DictionaryEntry(_keyProp.GetValue(_enumerator.Current, null),
                        _valueProp.GetValue(_enumerator.Current, null));
                }
            }
        ...
        private sealed class TypeDef
        {
            ...
            {
                _type = type;
                var members = options.SerializationOptions.HasFlag(JsonSerializationOptions.UseReflection)
                    ? EnumerateDefinitionsUsingReflection(true, type, options)
                    : EnumerateDefinitionsUsingTypeDescriptors(true, type, options);
                _serializationMembers = new List<MemberDefinition>(options.FinalizeSerializationMembers(type, members));

                members = options.SerializationOptions.HasFlag(JsonSerializationOptions.UseReflection)
                    ? EnumerateDefinitionsUsingReflection(false, type, options)
                    : EnumerateDefinitionsUsingTypeDescriptors(false, type, options);
                _deserializationMembers =
                    new List<MemberDefinition>(options.FinalizeDeserializationMembers(type, members));
            }
            ...
            public void WriteValues(TextWriter writer, object component, IDictionary<object, object> objectGraph,
                JsonOptions options)
            {
                var first = true;
                foreach (var member in _serializationMembers)
                {
                    ...
                    var skipDefaultValues =
                        options.SerializationOptions.HasFlag(JsonSerializationOptions.SkipDefaultValues);
            ...
            private static IEnumerable<MemberDefinition> EnumerateDefinitionsUsingReflection(bool serialization,
                Type type, JsonOptions options)
            {
                foreach (var info in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
                {
                    ...
                    ma.Accessor = (IMemberAccessor)Activator.CreateInstance(
                        typeof(PropertyInfoAccessor<,>).MakeGenericType(info.DeclaringType, info.PropertyType), info);
                    yield return ma;
                }
            ...
            private static IEnumerable<MemberDefinition> EnumerateDefinitionsUsingTypeDescriptors(bool serialization,
                Type type, JsonOptions options)
            {
                foreach (var descriptor in TypeDescriptor.GetProperties(type).Cast<PropertyDescriptor>())
                {
                    ...
                    if (options.SerializationOptions.HasFlag(JsonSerializationOptions.SkipGetOnly) &&
                    ...
                    ma.DefaultValue = defaultValue;
                    ma.Accessor =
                        (IMemberAccessor)Activator.CreateInstance(typeof(PropertyDescriptorAccessor), descriptor);
                    yield return ma;
                }
            }
        }
        ...
        private sealed class ICollectionTObject<T> : ListObject
        {
            ...
            public override void Add(object value, JsonOptions options = null)
            {
                if (value == null && typeof(T).IsValueType)
                    HandleException(
                        new JsonException("JSO0014: JSON error detected. Cannot add null to a collection of '" +
                                          typeof(T) + "' elements."), options);
        ...
        private delegate TResult JFunc<in T, out TResult>(T arg);

        private delegate void JAction<in T1, in T2>(T1 arg1, T2 arg2);
        ...
        private sealed class PropertyInfoAccessor<TComponent, TMember> : IMemberAccessor
        {
            ...
            public PropertyInfoAccessor(PropertyInfo pi)
            {
                ...
                if (set != null)
                    _set = (JAction<TComponent, TMember>)Delegate.CreateDelegate(typeof(JAction<TComponent, TMember>),
                        set);
            }
        ...
        private static class Conversions
        {
            ...
            public static object ChangeType(object input, Type conversionType, object defaultValue = null,
            ...
            private static bool TryChangeType(object input, Type conversionType, IFormatProvider provider,
                out object value)
            {
                if (conversionType == null)
                    throw new ArgumentNullException(nameof(conversionType));
                ...
                if (conversionType.IsArray || isGenericList)
                    ...
                    {
                        if (!isGenericList) elementType = conversionType.GetElementType();
                        ...
                        {
                            count++;
                            if (TryChangeType(obj, elementType, provider, out var element)) list.Add(element);
                        }
                        ...
                        if (count > 0 && list.Count > 0)
                        {
                            value = isGenericList
                                ? list
                                : list.GetType().GetMethod(nameof(List<object>.ToArray)).Invoke(list, null);
                            return true;
                        }
                    }
...
#pragma warning restore IDE0010 // Add missing cases
    ...
    {
        internal static DateTimeStyles _defaultDateTimeStyles = DateTimeStyles.AssumeUniversal |
...
#pragma warning restore CA1819 // Properties should not return arrays
        ...
        internal IDictionary<object, object> FinalObjectGraph =>
            ObjectGraph ?? new Dictionary<object, object>(Json.ReferenceComparer.Instance);
        ...
        public virtual IEnumerable<Json.MemberDefinition> FinalizeSerializationMembers(Type type,
        ...
        /// <param name="members">The members. May not be null.</param>
        /// <returns>A non-null list of members.</returns>
        public virtual IEnumerable<Json.MemberDefinition> FinalizeDeserializationMembers(Type type,
            IEnumerable<Json.MemberDefinition> members)
        {
            return members;
        }
        ...
        {
            return ((int)SerializationOptions).ToString();
        }
    }
    ...
    public class JsonEventArgs : EventArgs
    {
        ...
        public JsonEventArgs(TextWriter writer, object value, IDictionary<object, object> objectGraph,
            JsonOptions options)
        ...
        public JsonEventArgs(TextWriter writer, object value, IDictionary<object, object> objectGraph,
            JsonOptions options, string name, object component)
        {
            Options = options;
            Writer = writer;
    ...
    public enum JsonSerializationOptions
    {
        ...
        Default = UseXmlIgnore | UseScriptIgnore | SerializeFields | AutoParseDateTime | UseJsonAttribute |
                  SkipGetOnly | SkipDefaultValues | SkipZeroValueTypes | SkipNullPropertyValues | SkipNullDateTimeValues
    }
    ...
    public class JsonException : Exception
    {
        ...
        public static int GetCode(string message)
        {
            ...
            return int.TryParse(message.Substring(Prefix.Length, pos - Prefix.Length), NumberStyles.None,
